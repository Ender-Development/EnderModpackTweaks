...
public class EffectBeam extends Effect {
    ...
    public void render(float pticks) {
        ...
        RenderUtil.renderBeam(buffer, x, y, z, x2 * 0.1 + x * 0.9, y2 * 0.1 + y * 0.9, z2 * 0.1 + z * 0.9, r, g, b, 0f, r, g, b, a * getLifeCoeff(pticks), thickness,
                30f);
        RenderUtil.renderBeam(buffer, x2 * 0.1 + x * 0.9, y2 * 0.1 + y * 0.9, z2 * 0.1 + z * 0.9, x2 * 0.9 + x * 0.1, y2 * 0.9 + y * 0.1, z2 * 0.9 + z * 0.1, r, g, b,
                a * getLifeCoeff(pticks), r, g, b, a * getLifeCoeff(pticks), thickness, 30f);
        RenderUtil.renderBeam(buffer, x2 * 0.9 + x * 0.1, y2 * 0.9 + y * 0.1, z2 * 0.9 + z * 0.1, x2, y2, z2, r, g, b, a * getLifeCoeff(pticks), r, g, b, 0f, thickness,
                30f);
        tess.draw();
    }
}