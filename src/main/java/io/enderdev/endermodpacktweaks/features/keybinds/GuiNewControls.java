...
public class GuiNewControls extends GuiControls {
    ...
    private EnumSorting sortingType = EnumSorting.DEFAULT;
    ...
    public void initGui() {
        ...
        buttonConflict = new GuiButton(2906, this.width / 2 - 155 + 160, this.height - 29 - 24, 150 / 2, 20, translate("endermodpacktweaks.options.showconflicts"));
        buttonNone = new GuiButton(2907, this.width / 2 - 155 + 160 + 80, this.height - 29 - 24, 150 / 2, 20, translate("endermodpacktweaks.options.shownone"));
        buttonSorting = new GuiButton(2908, this.width / 2 - 155 + 160 + 80, this.height - 29 - 24 - 24, 150 / 2, 20, translate("endermodpacktweaks.options.sort") + ": " + sortingType.getName());
        boxSearchCategory = new GuiCheckBox(2909, this.width / 2 - (155 / 2) + 20, this.height - 29 - 37, translate("endermodpacktweaks.options.category"), false);
        boxSearchKey = new GuiCheckBox(2910, this.width / 2 - (155 / 2) + 20, this.height - 29 - 50, translate("endermodpacktweaks.options.key"), false);
        buttonToggleKeys = new GuiButton(2911, this.width / 2 - 155 + 160, this.height - 29 - 24 - 24, 150 / 2, 20, translate("endermodpacktweaks.options.togglefree"));
    ...
    public LinkedList<GuiListExtended.IGuiListEntry> sort(LinkedList<GuiListExtended.IGuiListEntry> list, EnumSorting type) {
        if (sortingType != EnumSorting.DEFAULT) {
    ...
    protected void actionPerformed(GuiButton button) {
        if (button.id == 200) {
            ...
            toggleFreeKeys = false;
            none = false;
            buttonNone.displayString = translate("endermodpacktweaks.options.shownone");
            if (!conflicts) {
                conflicts = true;
                buttonConflict.displayString = translate("endermodpacktweaks.options.showall");
                refreshKeys();
            } else {
                conflicts = false;
                buttonConflict.displayString = translate("endermodpacktweaks.options.showconflicts");
                refreshKeys();
            }
            ...
            if (!none) {
                none = true;
                buttonNone.displayString = translate("endermodpacktweaks.options.showall");
                refreshKeys();
            } else {
                none = false;
                buttonNone.displayString = translate("endermodpacktweaks.options.shownone");
            ...
            buttonSorting.displayString = translate("endermodpacktweaks.options.sort") + ": " + sortingType.getName();
            refreshKeys();
            ...
            buttonToggleKeys.displayString = translate("endermodpacktweaks.options.togglefree");
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks) {
        ...
        this.drawCenteredString(this.fontRenderer, translate("endermodpacktweaks.options.search"), this.width / 2 - (155 / 2), this.height - 29 - 39, 16777215);
        boolean flag = false;
        ...
        if (toggleFreeKeys) {
            ...
            final int[] y = {0};
            final int[] count = {0};
            fontRenderer.drawString(translate("endermodpacktweaks.options.availablekeys") + ":", width / 2, keyBindingList.top + 2, 0xFFFFFF);
            keyCodes.forEach(key -> {
                if (key >= 0) {
                    fontRenderer.drawString(Keyboard.getKeyName(key), keyBindingList.left + (x[0] * 65), keyBindingList.top + 12 + (y[0]++ * fontRenderer.FONT_HEIGHT), 0xFF55FF);
                } else {
                    switch (key + 100) {
                        case 0:
                            fontRenderer.drawString("Button 1", keyBindingList.left + (x[0] * 65), keyBindingList.top + 12 + (y[0]++ * fontRenderer.FONT_HEIGHT), 0x55FF55);
                            break;
                        case 1:
                            fontRenderer.drawString("Button 2", keyBindingList.left + (x[0] * 65), keyBindingList.top + 12 + (y[0]++ * fontRenderer.FONT_HEIGHT), 0x55FF55);
                            break;
                        case 2:
                            fontRenderer.drawString("Button 3", keyBindingList.left + (x[0] * 65), keyBindingList.top + 12 + (y[0]++ * fontRenderer.FONT_HEIGHT), 0x55FF55);
                            break;
